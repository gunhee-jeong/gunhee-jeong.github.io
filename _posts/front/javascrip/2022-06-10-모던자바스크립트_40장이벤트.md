...
div 태그를 클릭하면 ->  
div에서 이벤트 핸들러가 실행! 이벤트의 주인은 div이기 때문에 currentTarget === div  
그리고 div를 클릭했기 때문에 target === div!  
...
그래서 currentTarget과 target이 일치하지 않는 이벤트의 경우는  
Capture 단계와 Bubble 단계에서 선택적으로 이벤트가 발생할 수 있도록 하였다.  
