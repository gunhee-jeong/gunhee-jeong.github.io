...
//'A' ~ 'Z' 또는 'a' ~ 'z'가 한 번 이상 반복되늰 문자열을 전역 검색한다.
...
<u>\d는 숫자를 의미</u>한다. 즉, <span style="color:royalblue">\d는 [0-9]</span>와 같다.  
\D는 \d와 반대로 동작한다. 즉, <u>\D는 숫자가 아닌 문자를 의미</u>한다.  
...
이 방식은 특수 문자를 선택적으로 검사할 수도 있다는 장점이 있다.   -->

<!-- ```js
target.replace(/[^\-_+<>@\$%&\\\ \=\(\'\"]/gi).test(target); //true
``` -->
