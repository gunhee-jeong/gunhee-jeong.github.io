...
다시 말해, `제너레이터 객체`는 Symbol.iterator 메서드를 상속받는 <span class="teal">이터러블</span>이면서 value, done 프로퍼티를 갖는 이터레이터 리절트 객체를 반환하는 next 메서드를 소유하는 <span class="teal">이터레이터</span>다. 제너레이터 객체는 next 메서드를 가지는 이터레이터이므로 Symbol.iterator 메서드를 호출해서 별도로 이터레이터를 생성할 필요가 없다.
