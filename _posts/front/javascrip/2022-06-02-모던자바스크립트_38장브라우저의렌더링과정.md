...
HTML5부터 script 태그에 asyne와 defer 어트리뷰트가 추가되었다.  
...
여러 개의 script 태그에 aynce 어트리뷰트를 지정하면 script 태그의 순서와는 상관없이  
<span style="color:royalblue">로드가 완료된 자바스크립트부터 먼저 실행</span>되므로 <span style="color:blue">순서가 보장되지 않는다</span>.  
