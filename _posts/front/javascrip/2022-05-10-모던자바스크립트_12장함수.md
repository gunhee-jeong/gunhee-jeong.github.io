...
`순수 함수`는 <u>동일한 인수가 전달되면 언제나 동일한 값을 반환</u>하는 함수이다.  
즉, 순수 함수는 어떤 외부 상태에도 의존하지 않고 오직 매개변수를 통해 함수 내부로 전달된 인수에게만  
의존해 값을 생성해 반환한다.  

순수 함수는 일반적으로 최소 하나 이상의 인수를 전달받는다.  
<u>인수를 전달받지 않는 순수 함수</u>는 언제나 동일한 값을 반환하므로 <span style="color:royalblue">결국 상수와 마찬가지</span>다.  
<span style="color:blue">따라서 최소 하나 이상의 인수를 전달받지 않는 순수 함수는 그다지 의미가 없다</span>.  
...
함수가 외부 상태를 변경하면 <span style="color:royalblue">상태 변화를 추적하기 어려워</span>진다.  
