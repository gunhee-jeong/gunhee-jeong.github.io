...
`이터러블이면서 이터레이터인 객체`를 생성하면 <span class="teal">Symbol.iterator 메서드를 호출하지 않아도 된다</span>. 다음 객체는 Symbol.iterator 메서드와 next 메서드를 소유한 이터러블이면서 이터레이터다.Symbol.iterator 메서드는 this를 반환하므로 next 메서드를 갖는 이터레이터를 반환한다.
