...
프로퍼티 어트리뷰트는 자바스크립트 엔진이 관리하는 내부 상태 값인 내부 슬롯 [[Value]]], [[Writable]], [[Enumerable]]  
[[Configurable]]이다. 따라서 프로퍼티 어트리뷰트에 직접 접근할 수 없지만  
...
//[[Configurable]]의 값이 false인 경우 해당 프로퍼티를 삭젤할 수 없다.
...
객체 확장 금지란 프로퍼티 추가 금지를 의미한다. 즉, <span style="color:royalblue">확장이 금지된 객체는 프로퍼티 추가가 금지</span>된다.  
...
즉, <span style="color:royalblue">밀봉된 객체는 읽기와 쓰기만 가능</span>하다.  
