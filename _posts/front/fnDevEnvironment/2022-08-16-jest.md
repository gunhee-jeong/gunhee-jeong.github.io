...
## toBeNull()
`.toBeNull()`은 <u>.toBe와 동일하지만</u> <span class="blue">오류를 알려주는 것에 있어서 조금 더 자세하다</span>.  
무언가가 null인지 확인하려면 .toBeNull()을 사용하는 것이 좋다.  
...
예를 들어서 아래와 같은 코드일 때 <u>not.toBe()로 오류를 검사</u>하고 있다.  
실제로 "추가"라고 하는 것은 존재하지 않으므로 error로 결과가 나오게 될 것이다.  
...
test('App', () => {
  ...
  expect(getByText(/추가/)).not.toBe();
});
```

<img src="https://user-images.githubusercontent.com/87808288/185743376-e7782d12-8b40-4163-97a5-4559d99395d0.png" width="50%">  

그리고 이제 `not.toBeNull()`로 테스트를 진행해보면 <span class="royalblue">not.toBe()</span>와 같이 <u>결과적으로 error라고 판단</u>하지만 error의 원인을 알려주는 것에 있어서는 아래의 이미지와 같이 <span class="blue">보다 자세한 것</span>을 볼 수 있다.  
<img src="https://user-images.githubusercontent.com/87808288/185744236-442521e8-b7ba-4ee8-ae02-50f4940de3ef.png" width="70%">  
