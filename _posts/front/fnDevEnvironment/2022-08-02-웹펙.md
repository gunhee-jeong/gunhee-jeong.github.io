...
React Router에서 설정하지 않는 URL에 접속시 NotFoundPage로 이동하도록 미리 설정해주었다. 설정된 App 컴포넌트의 코드는 아래와 같다.
...
그런데 아래의 이미지와 같이 NotFoundPage로 가지 않고 에러가 발생했다.  
<img src="https://user-images.githubusercontent.com/87808288/190910924-ebcbcb79-e5ae-4d42-8da6-aa30d2e25088.png" width="90%">

이는 <u>서버에 존재하지 않는 /restaurant에 GET을</u> 보내기 때문이었다. <span class="forestgreen">브라우저가 서버에 요청을 보내지 못하도록</span> webpack의 설정을 추가하여 이를 해결할 수 있었다.

http://localhost:8080/에 대한 GET

devServer를 안해놓으면 NotFoundPage로 가라는게 App에 있는게 결국 이게 main.js에 들어있으니깐 다른 곳으로

어떤 곳을 치면 index.html로 가고 여기 연결된 main.jsx에 Aㅔㅔ
