...
- App은 <span class="forestgreen">상태가 어떻게 처리되는지 모르게</span> 한다.
...
위의 대표적인 사례로는 2014년 컨퍼런스에서 소개된 `페이스북의 채팅 버그` 사례가 있다. <u>읽지 않은 메시지 상태를 나타내는 카운터</u>를 확인하고 사용자가 메시지를 확인해도 <span class="forestgreen">어느새 숫자가 되살아나면서 사용자를 힘들게 한 버그</span>이다.(메시지를 확인해보아도 새로운 내용은 전혀없다.)

개발자가 버그를 수정해도 잠시 동안은 괜찮은 모습을 보이다가 계속해서 같은 버그가 보고되는 상황이 반복되었다. 그러면서 페이스북 팀은 <span class="forestgreen">설계에 기반한 근복적인 문제</span>가 있다고 판단하게 된다. 그리고 해결책으로 애플리케이션의 데이터가 단방향으로 흐르는 방법을 고안하게 된다. 플럭스 아키텍처의 구현체인 <span class="crimson">리덕스</span>는 <span class="mediumblue">애플리케이션을 위한 상태 컨테이너</span>로써 <span class="crimson">단방향 데이터 흐름</span>을 활용하여 <span class="forestgreen">시스템을 예측 가능하게 만들어서 시스템을 보완</span>하는 역할을 하게 된다.
...
## 상태값은 불변 객체다
<u>상태를 변경하는 유일한 방법</u>은 <span class="forestgreen">발생한 일을 설명</span>하는 <span class="crimson">객체인 action</span>을 보내는 것이다.
...
# 구조
## Action
<span class="forestgreen">상태값</span>은 <span class="crimson">오직 액션 객체에 의해서만 변경되어야</span> 한다.
- type: (string)
...
<u>동일한 상태를 공유하고 사용</u>해야 하는 여러 구성 요소가 있는 경우, 특히 <span class="forestgreen">해당 구성 요소가 애플리케이션의 다른 부분에</span> 있는 경우 <span class="mediumblue">단순성이 무너질 수</span> 있다.

이를 해결하는 한 가지 방법은 <u>컴포넌트에서 공유 상태를 추출</u>하여 컴포넌트 트리 <span class="forestgreen">외부의 중앙 위치에 두는 것</span>이다. 이를 통해 컴포넌트 트리는 큰 view가 되고 모든 구성 요소는 트리의 위치에 관계없이 상태에 액세스하거나 작업을 트리거할 수 있다.
...
<u>값을 변경할 수 없도록 업데이트하려면</u> 코드에서 기존 <span class="crimson">객체, 배열의 복사본</span>을 만든 다음 <span class="mediumblue">복사본을 수정</span>해야한다.
...
리덕스에선 <span class="forestgreen">기존 상태를 다른 상태로 변경하는 함수</span>를 `리듀서(reducer)`라고 한다. reducer의 구조는 아래와 같다.
...
<span class="crimson">reducer</span>는 <span class="mediumblue">이전 상태값</span>과 <span class="mediumblue">액션 객체</span>를 입력으로 받아서 <span class="mediumblue">새로운 상태값</span>을 만드는 <span class="forestgreen">순수 함수</span>이다. 순수 함수는 부수 효과(함수 외부의 상태를 변경시키는 것)를 발생시키지 않아야 한다. 순수 함수는 같은 입력값에 대해 항상 같은 값을 반환한다.

<span class="crimson">순수 함수</span>는 동일한 인수가 전달되면 언제나 동일한 값을 반환하는 함수이다. 즉, 순수 함수는 어떤 외부 상태에도 의존하지 않고 <u>오직 매개변수를 통해 함수 내부로 전달된 인수에게만 의존해 값을 생성해 반환</u>한다. 순수 함수는 일반적으로 최소 하나 이상의 인수를 전달받는다. 인수를 전달받지 않는 순수 함수는 언제나 동일한 값을 반환하므로 결국 상수와 마찬가지다. 따라서 최소 하나 이상의 인수를 전달받지 않는 순수 함수는 그다지 의미가 없다. 또한 <span class="mediumblue">순수 함수는 인수를 변경하지 않는 것이 기본</span>이다. 다시 말해, <span class="forestgreen">순수 함수는 인수의 불변성을 유지</span>한다.
...
리듀서는 <span class="mediumblue">기존 상태를 수정할 수 없다</span>. 대신 <span class="forestgreen">기존 상태를 복사</span>하고 복사된 값을 변경하여 변경할 수 없는 업데이트를 수행해야 한다.
...
아래의 Redux 애플리케이션의 <u>state</u>는 <span class="forestgreen">store</span> 라는 객체에 있다. store는 reducer를 전달하여 생성되며 현재 상태 값을 반환하는 getState 라는 메서드가 있다.
...
<span class="mediumblue">store.getState()</span>를 통해 <span class="forestgreen">현재 상태에 대한 액세스를 허용</span>한다.

<span class="mediumblue">store.dispatch()</span>를 통해 <span class="forestgreen">상태를 업데이트</span>할 수 있다.

<span class="mediumblue">store.subscribe(listener)</span>를 통해 <span class="forestgreen">리스너 콜백을 등록</span>한다. 그리고 store.subscribe(listener)에서 반환된 구독 취소 함수를 통해 수신기 등록 취소를 처리한다.
...
`subscribe`는 <u>리스너 콜백 배열을 유지</u>하고 <span class="forestgreen">새 콜백을 제거하는 함수를 반환</span>한다.

`dispatch`는 <span class="mediumblue">리듀서를 호출</span>하고, <span class="forestgreen">상태를 저장</span>하고, <span class="forestgreen">리스너를 실행</span>한다.
...
UI가 처음 렌더링될 때, UI 컴포넌트는 리덕스 스토어의 상태에 접근하여 해당 상태를 렌더링한다. 이후 <u>UI에서 상태가 변경</u>되면, <span class="forestgreen">앱은 디스패치를 실행해 액션을 일으킨다</span>. <u>새로운 액션을 받은 스토어</u>는 <span class="forestgreen">리듀서를 실행</span>하고 리듀서를 통해 나온 값을 새로운 상태로 저장한다. 서브스크라이브된 UI는 상태 업데이트로 변경된 데이터를 새롭게 렌더링한다.
