...
# 리액트 라우팅
<span class="crimson">Routing</span> 이란 <u>다른 경로(url 주소)에 따라 다른 View</u>를 보여주는 것을 말한다. React는 라이브러리로 이러한 기능이 내장되어 있지 않으므로 <span class="mediumblue">Router</span>를 이용하는 것이다.
...
위의 devServer와 같이 설정하면 <u>우리가 요청하는 모든 곳</u>은 무엇이든 <span class="forestgreen">index로 연결</span>이 된다.  
...
그리고 이때 element props에는 꼭 <span style="color:crimson">JSX로 설정</span>하여야 한다!
