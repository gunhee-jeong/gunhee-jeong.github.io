...
date: 2022-05-27T11:00:00+09:00
# sidebar:
# nav: "docs" #네비게이션에 있는 docs를 의미함
---
# 1. 문제
level이 'hard'인 과목들로만 이루어진 새로운 배열을, 해당과목만으로 이루어진 문자열로 반환하세요.  
...
solution(courses); // "Mathmatic, Science"
```
...
function solution (courses) {
  let list = courses.filter((aEle, aInd) => {
    if (aEle.level === 'hard') return true;
  });
  
  let answer = list.map(bEle => bEle.subject).join(', ');
