...
&nbsp;&nbsp; React란?  
뷰를 담당하는 라이브러리!  
가장 생태계가 활성화되어있기 때문에!(꾸준한 업데이트와 많은 정보!)

&nbsp;&nbsp; 특징
JSX, 컴포넌트, 버츄얼돔, 선언적  
선언적

프레임워크는 모든 것이 주어진 주방처럼 -> 넌 요리만 하면~  
근데 주어진 칼이 과도밖에 없을 수도 있는... 그런 곳이다.

&nbsp;&nbsp; CRA!  
노드르 설치하고 JS를 웹 밖에서도 사용가능하도록 만들고 -> tool 체인을 이용하여  
우리가 react로 구현할 라이브러리들을, 개발환경을 세팅해주는 키체인!  
이거 설치하는 법 정리하기

&nbsp;&nbsp; class 컴포넌트의 단점을 극복하는 함수 컴포넌트!  
훅스라는 것이 나오면서, 함수 컴포넌트를 많이ㅣ 사용한다!  
지금은 class 컴포넌트를 잘 지원하지 않느다.
